...
public enum TFGInitTask implements RegionTask {
    INSTANCE;

    @Override
    public void apply(RegionGenerator.Context context) {
        final Region region = context.region;
        final BitSet cell = new BitSet(RegionHelpers.size(region));
        ...
        context.region.setRegionArea(new Region.Point[(1 + maxX - minX) * (1 + maxZ - minZ)], minX, minZ, maxX, maxZ);
