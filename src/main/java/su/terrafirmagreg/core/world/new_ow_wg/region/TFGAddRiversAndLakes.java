...
public enum TFGAddRiversAndLakes implements RegionTask {
    INSTANCE;
    ...
    public void apply(RegionGenerator.Context context) {
        ...
        final TFGAddRiversAndLakes.RegionRiverGenerator riverGenerator = new TFGAddRiversAndLakes.RegionRiverGenerator(region);
    ...
    private void createInitialDrains(RegionGenerator.Context context, Region region, TFGAddRiversAndLakes.RegionRiverGenerator riverGenerator) {
        for (final var point : RegionHelpers.points(region)) {
            if (point.shore()) {
                IRegionPoint pt = (IRegionPoint) point;
                // Mark as a possible river source
                float bestAngle = findBestStartingAngle(region, context.random, pt.tfg$getIndex());
                if (!Float.isNaN(bestAngle)) {
                    final XoroshiroRandomSource rng = new XoroshiroRandomSource(context.random.nextLong());
                    riverGenerator.add(new River.Builder(rng, pt.tfg$getX() + 0.5f, pt.tfg$getZ() + 0.5f, bestAngle, RIVER_LENGTH, RIVER_DEPTH, RIVER_FEATHER));
                    point.setRiver();
                }
            }
        }
    ...
    private float findBestStartingAngle(Region region, RandomSource random, int index) {
        ...
        for (int dirX = -1; dirX <= 1; dirX++) {
            for (int dirZ = -1; dirZ <= 1; dirZ++) {
                ...
                final @Nullable Region.Point dirPoint = RegionHelpers.atOffset(region, index, 4 * dirX, 4 * dirZ);
