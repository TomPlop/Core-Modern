...
public class TFGChunkNoiseFiller extends TFGChunkHeightFiller {
    /**
    ...
    protected void updateLocalCaches(Object2DoubleMap<BiomeExtension> biomeWeights, BiomeExtension biomeAt, @Nullable RiverInfo info, double height, boolean couldBeSalty) {
        ...
        if (height <= TFCChunkGenerator.SEA_LEVEL_Y + 1 && info != null && info.normDistSq() < 1.1 && biomeAt.hasRivers()) {
            biomeAt = TFGBiomes.RIVER;
        }
        ...
        ((IChunkBaseBlockSource) baseBlockSource).tfg$useAccurateBiome(localX, localZ, biomeAt, biomeWeightAt, couldBeSalty);
    }
    ...
    private void setPerColumnDebugStates() {
        ...
        if (localBiomes[localX + 16 * localZ] == TFGBiomes.RIVER) {
            setDebugState(120, Blocks.MAGENTA_STAINED_GLASS); // River biome present + used in the local biomes
        }

        if (chunk.getNoiseBiome(localX >> 2, 120, localZ >> 2).value() == biomeSource.getBiomeFromExtension(TFGBiomes.RIVER).value()) {
