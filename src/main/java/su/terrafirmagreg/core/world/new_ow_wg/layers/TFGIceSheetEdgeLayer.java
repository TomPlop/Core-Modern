...
public enum TFGIceSheetEdgeLayer implements AdjacentTransformLayer {
    INSTANCE;
    ...
    public int apply(AreaContext context, int north, int east, int south, int west, int center) {
        ...
        if (center == KNOB_AND_KETTLE || center == PATTERNED_GROUND || center == INVERTED_PATTERNED_GROUND || center == STONE_CIRCLES) {
            ...
            } else if ((matcher.test(TFGLayers::isFlatIceSheet))) {
        ...
        if (center == ICE_SHEET_OCEANIC_MOUNTAINS) {
            if (matcher.test(TFGIceSheetEdgeLayer::isNotIceSheet)) {
        ...
        if (center == LAKE && matcher.test(TFGLayers::isFlatIceSheet)
                && !matcher.test(i -> i == ICE_SHEET_MOUNTAINS || i == ICE_SHEET_MOUNTAINS_EDGE || i == ICE_SHEET_OCEANIC_MOUNTAINS || i == ICE_SHEET_OCEANIC)) {
            return SUBGLACIAL_LAKE;
        }
        if (TFGLayers.isFlatIceSheet(center) && matcher.test(i -> i == MELTWATER_LAKE)) {
            if (matcher.test(TFGIceSheetEdgeLayer::isNotIceSheet)) {
                return SUBGLACIAL_LAKE;
            }
        }

        if (TFGLayers.isFlatIceSheet(center) && matcher.test(i -> i == OCEAN || i == OCEAN_REEF || i == DEEP_OCEAN || i == DEEP_OCEAN_TRENCH || i == ICE_SHEET_SHORE)) {
    ...
    public static boolean isNotIceSheet(int value) {
        return value != ICE_SHEET &&
                ...
                value != SUBGLACIAL_LAKE &&
                value != ICE_SHEET_MOUNTAINS &&
                value != ICE_SHEET_OCEANIC_MOUNTAINS /*&&
