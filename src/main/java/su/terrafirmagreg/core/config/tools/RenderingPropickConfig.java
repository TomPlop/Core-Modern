...
public record RenderingPropickConfig(PropickConfig inner, ForgeConfigSpec.BooleanValue preciselyRenderVein){
    public static RenderingPropickConfig build(ForgeConfigSpec.Builder builder, Metal.Default metal, int searchLength, int searchWidth, boolean preciselyRenderVein) {
        return new RenderingPropickConfig(
                PropickConfig.build(builder, metal, searchLength, searchWidth),
                builder.comment(String.format("\nShould the %s Prospector's Pick render particles per vein (vague)?\nSetting false will render particles per block (precise). Default: %s", ConfigHelpers.toTitleCase(metal.getSerializedName()), preciselyRenderVein))
                .define(String.format("%sProspectorRender", ConfigHelpers.toTitleCase(metal.getSerializedName(), false)), preciselyRenderVein)
        );
    }
}
